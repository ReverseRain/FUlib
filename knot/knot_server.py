    ...
    def _load_model(self, cluster_id, filename=None, location=None):
        ...
        if filename is not None:
            ...
            if not os.path.exists(model_path):
                logging.error("[ERROR] Model file not found: %s", model_path)
                raise FileNotFoundError(f"[ERROR] Model file not found: {model_path}")

            # **检查文件是否可读**
            if not os.access(model_path, os.R_OK):
                logging.error("[ERROR] No read permission for: %s", model_path)
                raise PermissionError(f"[ERROR] No read permission for: {model_path}")

            # **监测文件是否被删除**
            time.sleep(0.5)  # 等待一小段时间，防止文件刚被创建就消失
            if not os.path.exists(model_path):
                logging.error("[ERROR] Model file was deleted after checking: %s", model_path)
                raise FileNotFoundError(f"[ERROR] Model file was deleted after checking: {model_path}")
    ...
    def _save_model(self, cluster_id, filename=None, location=None):
        ...
        if filename is not None:
            ...
            if not os.access(model_path, os.W_OK):
                logging.error("[ERROR] No write permission for: %s", model_path)
                raise PermissionError(f"[ERROR] No write permission for: {model_path}")

            # **检查文件是否被其他进程占用**
            if os.path.exists(model_path):
                try:
                    with open(model_path, "a"):  # 试图以追加模式打开文件
                        pass
                except PermissionError:
                    logging.error("[ERROR] File is locked or in use: %s", model_path)
                    raise PermissionError(f"[ERROR] File is locked or in use: {model_path}")
        ...
        try:
            ...
            if not os.path.exists(backup_dir):
                os.makedirs(backup_dir)  # 确保备份目录存在

            backup_path = os.path.join(backup_dir, os.path.basename(model_path))
            shutil.copy(model_path, backup_path)  # 复制文件
            logging.info("[Server #%d] Model backup saved to %s.", os.getpid(), backup_path)
            print(f"[DEBUG] Model backup saved to: {backup_path}")
        except Exception as e:
            logging.error("[ERROR] Failed to load model: %s", str(e))
