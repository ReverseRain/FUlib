...
class NoiseGenerator:
    ...
    def generate_for_client(self, client_train_loader, steps=200, lr=0.1):
        ...
        for batch_idx, (x, y) in enumerate(client_train_loader):
            ...
            # 2. 针对当前 Batch 包含的每一个类别，分别调用 generate_for_class
...
def aggregate_repair_noises(
        client_noises_list,
        client_labels_list,
        client_remaining_sizes
):
    """
    Jellyfish Section 4.5: Aggregation of N_r.

    论文说明多个客户端提交 N_r^i 时，客户端 i 的贡献权重与
    remaining-data size |D_r^i| 成正比，类似 FedAvg。

    当前工程的 generate_for_client 是“一条本地训练样本 -> 一条 proxy”，
    因此客户端 i 实际生成的 proxy 数量通常等于 |D_r^i|。
    将所有 N_r^i 在样本维度 union/concat 后做均匀 minibatch 训练，
    就会自然得到：
        contribution_i ~= |D_r^i| / sum_k |D_r^k|

    这里显式计算 expected / actual weight 并打印，便于验证该假设。
    不对 proxy 像素做 weighted average，因为不同类别/样本的 noise
    不能直接逐像素平均，否则会破坏 error-minimization 激活结构。
    ...
    """
    if not (
            len(client_noises_list)
            == len(client_labels_list)
            == len(client_remaining_sizes)
    ):
        raise ValueError(
            "Repair aggregation input length mismatch."
        )

    if len(client_noises_list) == 0:
        raise ValueError(
            "No repair proxy N_r received."
        )
    ...
    for noises, labels in zip(
        ...
        if len(noises) != len(labels):
            raise ValueError(
                "Repair proxy batch/label length mismatch."
            )

        if len(noises) == 0:
            all_client_noises.append(None)
            all_client_labels.append(None)
            actual_counts.append(0)
            continue

        client_noise = torch.cat(
            [
                batch.detach().cpu()
                for batch in noises
            ],
        ...
        if client_noise.shape[0] != client_label.shape[0]:
            raise ValueError(
                "Repair proxy sample/label count mismatch."
            )

        all_client_noises.append(
            client_noise
        )
        all_client_labels.append(
            client_label
        )
        actual_counts.append(
            int(client_label.numel())
        )

    valid_noises = [
        item
        for item in all_client_noises
        if item is not None
    ]
    valid_labels = [
        item
        for item in all_client_labels
        if item is not None
    ]

    if len(valid_noises) == 0:
        raise ValueError(
            "All repair proxy sets are empty."
        )
    ...
    # 全局随机重排，只改变样本顺序，不改变任何 proxy 的幅值。
    total_samples = int(
        aggregated_labels.numel()
    )
    ...
    expected_total = max(
        sum(
            int(v)
            for v in client_remaining_sizes
        ),
        1
    )
    actual_total = max(
    ...
    # print(
    #     "\n[Stage4 Aggregation] "
    #     "FedAvg-style remaining-data contribution:"
    # )
    ...
    for index, (
        ...
        info = {
            ...
            "actual_proxy_weight": float(
                actual_weight
            ),
        }
        aggregation_info.append(info)

    #     print(
    #         f"  source[{index}] "
    #         f"|D_r|={int(expected_count)}, "
    #         f"N_r={int(actual_count)}, "
    #         f"expected={expected_weight:.4%}, "
    #         f"actual={actual_weight:.4%}"
    #     )
    #
    # print(
    #     f"[Stage4 Aggregation] "
    #     f"Global N_r samples={total_samples}"
    # )
...
def disentangle_loss(feature):
    """
    Jellyfish Knowledge Disentanglement Loss
    L_dis = ||Fconv||1
    """
    return torch.mean(
        torch.abs(feature)
...
# 第二步知识解耦的损失函数，对alpha以上的通道计算L1范数最小化
def threshold_disentangle_loss(feature, alpha=0.9):
    """
    Parameters
    ----------
    feature : Tensor
        最后卷积层输出，shape = [B, C, H, W]
    alpha : float
        通道保留比例 alpha。
        例如 alpha=0.9：
            保留 top 90% 高激活通道，
    ...
    if feature.ndim != 4:
        raise ValueError(
            f"Expected F_conv with shape [B,C,H,W], got {tuple(feature.shape)}"
        )

    if not (0.0 < alpha < 1.0):
        raise ValueError(f"alpha must be in (0,1), got {alpha}")

    # 对每一个样本、每一个通道计算空间维度上的 L1 norm：
    # channel_norm[b, c] = ||F_conv[b,c,:,:]||_1
    channel_norm = torch.sum(
        torch.abs(feature),
        dim=(2, 3)
    )  # [B, C]

    _, C = channel_norm.shape

    # alpha 是保留比例。
    # 要保留 top alpha，因此被抑制的是 bottom (1-alpha)。
    # 阈值应位于 (1-alpha) 分位点，而不是 alpha 分位点。
    ...
    # otherwise        -> 0
    low_channel_mask = (
            channel_norm < threshold
    ).to(channel_norm.dtype)

    # Eq.(6)
    selected_norm = channel_norm * low_channel_mask

    denominator = max((1.0 - alpha) * C, 1e-12)

    # 先按样本求 loss，再对 batch 取均值。
    loss_per_sample = (
            selected_norm.sum(dim=1)
            / denominator
    )
